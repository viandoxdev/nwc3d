...
def parse_device_information(device_information_file):
  ...
  print(f"ram: {external_apps_ram_end - external_apps_ram_start}")
  print(f"flash: {external_apps_flash_end - external_apps_flash_start}")
...
def load_elf(elf_file, device_information, app_index):
  ...
  subprocess.check_output(["arm-hardfloat-eabi-objcopy", "-O", "binary", elf_file, bin_file])
